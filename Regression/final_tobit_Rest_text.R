...
dat1S = dat1S[ , !(names(dat1S) %in% c('review_id', 'business_id', 'user_id', 'date', 'text', 'useful', 'cool', 'funny','usr_total_votes', 'yelping_since', 'fans', 'usr_avg_stars', 'usr_review_count', 'elite_count', 'usr_score', 'avg_nw_score','max_nw_score','friend_count','posted_days'))]

dat1S$yhat <- predict(bsTobitTextText, dat1S)
(r <- with(dat1S, cor(yhat, total_votes)))
r^2

sqrt(mean((dat1S$total_votes - predict(bsTobitTextText, dat1S)) ^ 2))
mean((dat1S$total_votes - predict(bsTobitTextText, dat1S)) ^ 2)
sum(abs(dat1S$total_votes - predict(bsTobitTextText, dat1S)))/dim(dat1S)[1]

# R2<- 1-(bsTobitTextText$deviance/bsTobitTextText$null.deviance)
# R2
# 
mnull <- update(bsTobitTextText, . ~ 1)
pchisq(2 * (logLik(bsTobitTextText) - logLik(mnull)), df = 3, lower.tail = FALSE)

# dat$yhat <- fitted(bsTobitTextText)[,1]
# dat$rr <- resid(bsTobitTextText, type = "response")
# dat$rp <- resid(bsTobitTextText, type = "pearson")[,1]
# 
# par(mfcol = c(2, 3))
# 
# with(dat, {
