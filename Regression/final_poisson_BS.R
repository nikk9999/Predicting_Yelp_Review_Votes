...
dat1S <- read.csv("finalData/test_reviews_BS.csv")
dat1S = na.omit(dat1S)
...
dat1S$yhat <- predict(bsPoisson, dat1S)
(r <- with(dat1S, cor(yhat, total_votes)))
r^2

sqrt(mean((dat1S$total_votes - predict(bsPoisson, dat1S)) ^ 2))
mean((dat1S$total_votes - predict(bsPoisson, dat1S)) ^ 2)
# R2<- 1-(bsPoisson$deviance/bsPoisson$null.deviance)
# R2
# 
mnull <- update(bsPoisson, . ~ 1)
pchisq(2 * (logLik(bsPoisson) - logLik(mnull)), df = 3, lower.tail = FALSE)

# dat$yhat <- fitted(bsPoisson)[,1]
# dat$rr <- resid(bsPoisson, type = "response")
# dat$rp <- resid(bsPoisson, type = "pearson")[,1]
# 
# par(mfcol = c(2, 3))
# 
# with(dat, {
