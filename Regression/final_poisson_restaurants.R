...
dat1S$yhat <- predict(restPoisson, dat1S)
(r <- with(dat1S, cor(yhat, total_votes)))
r^2

sqrt(mean((dat1S$total_votes - predict(restPoisson, dat1S)) ^ 2))
mean((dat1S$total_votes - predict(restPoisson, dat1S)) ^ 2)
# R2<- 1-(restPoisson$deviance/restPoisson$null.deviance)
# R2
# 
mnull <- update(restPoisson, . ~ 1)
pchisq(2 * (logLik(restPoisson) - logLik(mnull)), df = 3, lower.tail = FALSE)

# dat$yhat <- fitted(restPoisson)[,1]
# dat$rr <- resid(restPoisson, type = "response")
# dat$rp <- resid(restPoisson, type = "pearson")[,1]
# 
# par(mfcol = c(2, 3))
# 
# with(dat, {
