...
dat1S$yhat <- predict(bsTobit, dat1S)
(r <- with(dat1S, cor(yhat, total_votes)))
r^2

sqrt(mean((dat1S$total_votes - predict(bsTobit, dat1S)) ^ 2))
mean((dat1S$total_votes - predict(bsTobit, dat1S)) ^ 2)
# R2<- 1-(bsTobit$deviance/bsTobit$null.deviance)
# R2
# 
mnull <- update(bsTobit, . ~ 1)
pchisq(2 * (logLik(bsTobit) - logLik(mnull)), df = 3, lower.tail = FALSE)

# dat$yhat <- fitted(bsTobit)[,1]
# dat$rr <- resid(bsTobit, type = "response")
# dat$rp <- resid(bsTobit, type = "pearson")[,1]
# 
# par(mfcol = c(2, 3))
# 
# with(dat, {
