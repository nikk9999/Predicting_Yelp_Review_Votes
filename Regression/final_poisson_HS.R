...
dat1S$yhat <- predict(hsPoisson, dat1S)
(r <- with(dat1S, cor(yhat, total_votes)))
r^2

sqrt(mean((dat1S$total_votes - predict(hsPoisson, dat1S)) ^ 2))
mean((dat1S$total_votes - predict(hsPoisson, dat1S)) ^ 2)
# R2<- 1-(hsPoisson$deviance/hsPoisson$null.deviance)
# R2
# 
mnull <- update(hsPoisson, . ~ 1)
pchisq(2 * (logLik(hsPoisson) - logLik(mnull)), df = 3, lower.tail = FALSE)

# dat$yhat <- fitted(hsPoisson)[,1]
# dat$rr <- resid(hsPoisson, type = "response")
# dat$rp <- resid(hsPoisson, type = "pearson")[,1]
# 
# par(mfcol = c(2, 3))
# 
# with(dat, {
