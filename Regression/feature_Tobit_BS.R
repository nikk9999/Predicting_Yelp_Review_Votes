dat <- read.csv("finalData/train_reviews_BS.csv")
...
dat1S$yhat <- predict(bsTobit1, dat1S)
(r <- with(dat1S, cor(yhat, total_votes)))
r^2

sqrt(mean((dat1S$total_votes - predict(bsTobit1, dat1S)) ^ 2))
mean((dat1S$total_votes - predict(bsTobit1, dat1S)) ^ 2)
sum(abs(dat1S$total_votes - predict(bsTobit1, dat1S)))/dim(dat1S)[1]
...
# r2 = 1-(numer/denom)
# r2
# R2<- 1-(bsTobit1$deviance/bsTobit1$null.deviance)
# R2
# 
mnull <- update(bsTobit1, . ~ 1)
pchisq(2 * (logLik(bsTobit1) - logLik(mnull)), df = 3, lower.tail = FALSE)

# dat$yhat <- fitted(bsTobit1)[,1]
# dat$rr <- resid(bsTobit1, type = "response")
# dat$rp <- resid(bsTobit1, type = "pearson")[,1]
# 
# par(mfcol = c(2, 3))
# 
# with(dat, {
