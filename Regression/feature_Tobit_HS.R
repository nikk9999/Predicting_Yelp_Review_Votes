...
dat1S$yhat <- predict(hsTobit1, dat1S)
(r <- with(dat1S, cor(yhat, total_votes)))
r^2

sqrt(mean((dat1S$total_votes - predict(hsTobit1, dat1S)) ^ 2))
mean((dat1S$total_votes - predict(hsTobit1, dat1S)) ^ 2)
sum(abs(dat1S$total_votes - predict(hsTobit1, dat1S)))/dim(dat1S)[1]
...
# r2 = 1-(numer/denom)
# r2
# R2<- 1-(hsTobit1$deviance/hsTobit1$null.deviance)
# R2
# 
mnull <- update(hsTobit1, . ~ 1)
pchisq(2 * (logLik(hsTobit1) - logLik(mnull)), df = 3, lower.tail = FALSE)

# dat$yhat <- fitted(hsTobit1)[,1]
# dat$rr <- resid(hsTobit1, type = "response")
# dat$rp <- resid(hsTobit1, type = "pearson")[,1]
# 
# par(mfcol = c(2, 3))
# 
# with(dat, {
